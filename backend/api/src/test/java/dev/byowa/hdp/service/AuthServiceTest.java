...
class AuthServiceTest {
    ...
    void register_success_createsUserAndReturnsToken() {
        ...
        when(jwtService.generateToken("alice", Role.PATIENT.name(), "Alice Doe")).thenReturn("token-123");
    ...
    void register_invokesJwtWithUsernameAndRole() {
        ...
        when(jwtService.generateToken("carol", Role.PATIENT.name(), "Carol Smith")).thenReturn("tkn");

        authService.register(req);

        // ensure user saved and jwt called with correct args
        verify(userRepository).save(any(User.class));
        verify(jwtService).generateToken("carol", Role.PATIENT.name(), "Carol Smith");
    }
    ...
    void login_success_returnsToken() {
        ...
        when(jwtService.generateToken("bob", Role.ADMIN.name(), null)).thenReturn("jwt");
    ...
    void login_invokesJwtWithUsernameAndResolvedRole() {
        ...
        when(jwtService.generateToken("dave", Role.PATIENT.name(), null)).thenReturn("tok");

        AuthResponse resp = authService.login(req);
        assertEquals("tok", resp.getToken());
        verify(jwtService).generateToken("dave", Role.PATIENT.name(), null);
    }
